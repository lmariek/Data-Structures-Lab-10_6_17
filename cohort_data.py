...
def unique_houses(filename):
    ...
    >>> houses = unique_houses("cohort_data.txt")
    >>> expected = set(['Gryffindor', 'Hufflepuff', "Dumbledore's Army", 'Ravenclaw', 'Slytherin'])
    >>> houses ^ expected 
    set([])

    """

    hp_info = open(filename)

    all_houses = set()

    for line in hp_info:
        hp_info_rows = line.split("|")
        all_houses.add(hp_info_rows[2])
    all_houses.remove('')
...
def sort_by_cohort(filename):
    ...
    for line in hp_info:
        line = line.rstrip()
        hp_info_rows = line.split("|")
        cohort = hp_info_rows[4]
        student_name = hp_info_rows[0] + ' ' + hp_info_rows[1]
